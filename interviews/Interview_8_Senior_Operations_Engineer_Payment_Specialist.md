...
the frontend sends a book transaction request to ABFR just like it would for any internal backoffice. ABFR figures out where to route it. 
...
Senior Operations Engineer: Exactly. That is what the adapter does. ABFR receives a standardised request in our internal format and translates it to whatever ABN AMRO expects. 
...
Q4: Was there any discussion about whether consent should be validated inside ABFR or somewhere else?
 
Payment Specialist: Yes, we discussed this. The question was whether ABFR should check consent or whether the frontend should do it before sending the request. 
We decided ABFR is the right place because ABFR is the gateway. If consent is checked at the frontend, there is a risk that a request slips through if the frontend logic is bypassed or misconfigured.
...
Q5: Was there a discussion about where to store the OAuth2 access token? Should ABFR cache it or request a new one every time?
...
The TLS certificate does most of the heavy lifting for security anyway.

 
Q6: Were there any concerns about what happens if the ABN AMRO API is unavailable?
 
Senior Operations Engineer: Yes, we discussed fallback behaviour. If the ABN AMRO API is down, ABFR should return a clear error to the frontend rather than timing out silently.
...
Senior Operations Engineer: We also discussed monitoring. If we route calls through ABFR, we get a central place to log all external API calls. 
